        ...
        placeHolderDiv.on('click',
            ...
            function (event) {
                ...
                $.ajax({
                    ...
                    contentType: false,
                    success: function (data) {
                        const userRoleUpdateAjaxModel = jQuery.parseJSON(data);
                        console.log(userRoleUpdateAjaxModel);
                        const newFormBody = $('.modal-body', userRoleUpdateAjaxModel.RoleAssignPartial);
                        placeHolderDiv.find('.modal-body').replaceWith(newFormBody);
                        const isValid = newFormBody.find('[name="IsValid"]').val() === 'True';
                        if (isValid) {
                            const id = userRoleUpdateAjaxModel.UserDto.User.Id;
                            const tableRow = $(`[name="${id}"]`);
                            //placeHolderDiv.find('.modal').modal('hide');
                            toastr.success(`${userRoleUpdateAjaxModel.UserDto.Message}`, "Başarılı İşlem!");
                        } else {
                            let summaryText = "";
                            $('#validation-summary > ul > li').each(function () {
                                let text = $(this).text();
